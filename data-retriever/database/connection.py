...
class DBConnectionManager:
    ...
    def init_pool(cls, minconn: Optional[int] = None, maxconn: Optional[int] = None) -> SimpleConnectionPool:
        ...
        logger.info("DB_POOL_INITIALIZED|min=%d|max=%d", min_conn, max_conn)
        return cls._pool
    ...
    def _release_connection_safely(
        ...
        if error and isinstance(error, (OperationalError, InterfaceError)):
            close_conn = True
            logger.warning("DB_CLOSING_BAD_CONNECTION|error_type=%s", type(error).__name__)
    ...
    def get_pool_stats(cls) -> dict:
        ...
        try:
            ...
            logger.error("DB_POOL_STATS_ERROR|error=%s", exc)
