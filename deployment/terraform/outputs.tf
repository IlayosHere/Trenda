...
output "deployment_summary" {
  value = <<-EOF
    ...
    VM Instance:
      ...
      Internal IP: ${google_compute_instance.main.network_interface[0].network_ip}
      Type:        ${var.vm_machine_type}
    
    Database:
      Instance:   ${google_sql_database_instance.main.name}
      Private IP: ${google_sql_database_instance.main.private_ip_address}
    ...
    SSH Access:
      gcloud compute ssh ${google_compute_instance.main.name} --zone=${var.zone} --tunnel-through-iap
